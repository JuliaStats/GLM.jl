...
@testset "Linear model ftest with $dmethod method with dropcollinear=$drop" for (
dmethod, drop) in itr
    model_0 = lm(
        @formula(y~x1), df; wts = aweights(df.w), method = dmethod, dropcollinear = drop)
...
@testset "GLM: Binomial with LogitLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
...
@testset "GLM: Binomial with ProbitLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
...
@testset "GLM: Binomial with CauchitLink link - AnalyticWeights - method $dmethod" for (
dmethod, drop) in itr
    model = glm(
...
@testset "GLM: Binomial with CloglogLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
    model = glm(
...
@testset "GLM: Gamma with InverseLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
    model = glm(@formula(lot1~1 + u), clotting, Gamma(), InverseLink(),
        wts = aweights(clotting.w), method = dmethod, dropcollinear = drop, atol = 1e-07, rtol = 1e-08)
...
@testset "GLM: Gamma with IdentityLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
...
@testset "GLM: Gamma with LogLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
    model = glm(@formula(lot1~1 + u), clotting, Gamma(), LogLink(),
        wts = aweights(clotting.w), method = dmethod, dropcollinear = drop, atol = 1e-09, rtol = 1e-09)
...
@testset "GLM: InverseGaussian with InverseSquareLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
...
@testset "GLM:  NegativeBinomial with LogLink link - AnalyticWeights - method: dmethod" for (
dmethod, drop) in itr
    ...
    @test GLM.momentmatrix(model)≈[-3.866780529709063 -0.0 -3.866780529709063 -0.0 -0.0 -0.0 -3.866780529709063
...
@testset "GLM:  NegativeBinomial with LogLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
...
@testset "GLM:  NegativeBinomial with SqrtLink link - AnalyticWeights with $dmethod method" for (
dmethod, drop) in itr
    model = glm(@formula(Days~Eth + Sex + Age + Lrn), quine, NegativeBinomial(2),
    ...
    @test GLM.momentmatrix(model)≈[-1.4294351675636041 -0.0 -1.4294351675636041 -0.0 -0.0 -0.0 -1.4294351675636041
